...
- `apps/` holds the externally exported applications, assembled from Client / Host mixtures.
    ...
    - The [Electron desktop host](../feature/2026-08-17-configurable-electron-desktop-host.md) reuses the same Web assembly and loopback carrier; an IPC carrier remains a transport replacement when a native-only protocol justifies it.
...
| Carrier layer | `dsh-host-webserver` | Web HTTP and upgrade: static serving + `/api/*`→handler forwarding + WebSocket upgrade route + close semantics; plugin bundle endpoint + `__DSH_BOOT__` manifest injection (fed by the web plugin registry) | Browser and Electron loopback access; zero workspace dependencies (the registry arrives by structural injection) |
...
1. **Pick a fetch transport**: same-origin HTTP, in-process `host.handler.fetch` injection, or a transport-aspect subclass when a different process boundary requires one.
...
| Electron desktop host | apps/desktop | `WebApiClient` over an OS-assigned loopback port | reuses the browser assembly; see the [desktop-host decision](../feature/2026-08-17-configurable-electron-desktop-host.md) |
