...
- `apps/` 作为对外导出的应用入口，可以由 Client / Host 混合组装。
    ...
    - [Electron 桌面宿主](../feature/2026-08-17-configurable-electron-desktop-host.md)复用同一套 Web 组合与回环载体；只有原生专属协议证明有必要时，IPC 载体才替换该传输。
...
| 承载层 | `dsh-host-webserver` | Web HTTP 与 upgrade：静态服务 + `/api/*`→handler 转发 + WebSocket upgrade route + close 语义；插件 bundle 端点 + `__DSH_BOOT__` manifest（元数据清单）注入（由 web 插件注册表供给） | 浏览器和 Electron 回环访问；零 workspace 依赖（注册表经结构注入到达） |
...
1. **选择 fetch 传输**：同源 HTTP、进程内 `host.handler.fetch` 注入，或在不同进程边界确有需要时使用传输切面子类。
...
| Electron 桌面宿主 | apps/desktop | 通过操作系统分配的回环端口使用 `WebApiClient` | 复用浏览器组合；详见[桌面宿主决策](../feature/2026-08-17-configurable-electron-desktop-host.md) |
