...
When a confining executor is mounted, `bash` advertises paired `sandbox_permissions` and `justification` fields. The schema exposes the full closed escalation vocabulary because effective mode is per-session; execution normalizes a recognized non-widening target to the standing policy and sends only a genuinely wider target to approval. Approval resolves before execution. `allowed-once` stamps the granted mode onto only that request, while `rejected`, `cancelled`, `unavailable`, a missing approval service, or a missing agent all fail closed with distinct results. No grant is persisted.

The escalation parameter descriptions state that a recognized target equal to or narrower than the effective mode is inert. Each tool resolves the call's effective mode before interpreting the fields: a non-widening target and its justification normalize to no escalation and execute under the standing policy without approval, while a genuinely wider target validates its paired reason before approval. Under `danger-full-access`, every field combination remains inert. `approveEscalation` retains an independent strict-widening check for callers that bypass this normalization.
...
- **Fail recognized non-widening escalation fields** — rejected: model adapters may fill optional tool fields despite explicit schema guidance, and rejecting inert arguments can produce an unbounded retry loop without protecting any additional authority. Normalization preserves the standing policy because an equal or narrower target cannot add authority; the independent approval check still rejects a non-widening request from callers that bypass the tool resolver.
...
- The escalation fields exist exactly when the mounted executor confines; a confined request that is not strictly wider than the call's effective mode fails closed with a correction the caller can apply and prompts no one; under `danger-full-access`, either field is inert, no approval is requested, and the call executes with the standing policy; a deployment with no ApprovalService fails actionable escalating calls closed and leaves plain calls untouched.
...
- **The advertised target set is static while the effective mode is per-session** (schemas are registry-global) — a session already at or above an advertised target is still offered that field value. Harmless by construction: the tool resolver ignores a recognized non-widening target and preserves the standing policy without prompting, while the approval helper independently requires strict widening from any caller that bypasses the resolver.
