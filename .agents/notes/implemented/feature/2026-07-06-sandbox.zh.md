...
当约束执行器被挂载时，`bash` 公布配对的 `sandbox_permissions` 和 `justification` 字段。schema 暴露完整的封闭升级词汇，因为有效模式是按会话的；执行会把已知的未拓宽目标归一化为常驻策略，只把真正更宽的目标送入批准。批准在执行之前解析。`allowed-once` 仅将授权模式盖章到该请求上，而 `rejected`、`cancelled`、`unavailable`、缺失的 approval 服务或缺失的 agent 都以各自不同的结果文本失败关闭。授权不持久化。

升级参数描述会说明已知目标若等于或窄于有效模式就不起作用。每个工具都会先解析调用的有效模式，再解释这些字段：未拓宽目标及其理由会被归一化为无需升级，不发起批准并按常驻策略执行；真正更宽的目标则在批准前校验配对理由。在 `danger-full-access` 下，任意字段组合仍不起作用。`approveEscalation` 仍保留独立的严格拓宽检查，防御绕过该归一化的调用方。
...
- **快照：** 固定原子化的当前策略上下文和两个脚本化的 approval 分支。一个真实 ACP 示例场景把会话放在用户主目录下，同时让部署后备根目录指向 `/tmp`，然后固定 workspace-write 运行时上下文消息与一次成功的、由部署选定的变更；这能区分会话根目录解析与进程后备值，而不依赖 runner 特定的拒绝文本。手工编写的 Full access 与 workspace-write 场景会通过组装后的文件系统工具重放冗余升级字段，并固定无需批准的成功修改。一个模拟 Landlock 部分强制执行行为的 POSIX 提供方会在组装后的应用中固定直接执行 bash `false` 时仍得到普通子进程结果，并固定提供方可执行文件缺失时在前台／后台均为基础设施失败。
...
- **让已知的未拓宽升级字段失败关闭**：否决。即使 schema 已明确引导，模型适配器仍可能填充可选工具字段；拒绝这些不起作用的参数会形成无界重试循环，却没有保护任何额外权威。归一化会保留常驻策略，因为相等或更窄的目标无法增加权威；独立批准检查仍会拒绝绕过工具解析器的未拓宽请求。
...
- 升级字段恰好在已挂载的执行器约束时存在；不严格宽于调用有效模式的受限请求会以带有明确修正指引的错误文本失败关闭，且不提示任何人；在 `danger-full-access` 下任一字段都不起作用，不发起批准，调用按常驻策略执行；没有 ApprovalService 的部署会让确实需要升级的调用失败关闭，对普通调用不影响。
...
- **公布的目标集是静态的，而有效模式是按会话的**（schema 是注册表全局的）——已处于某个目标或更宽模式的会话仍会看到该字段值。构造上无害：工具解析器会忽略已知的未拓宽目标并保留常驻策略且不发起提示，而批准辅助函数仍会对绕过解析器的调用方独立要求严格拓宽。
