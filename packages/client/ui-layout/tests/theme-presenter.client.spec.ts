...
function snapshot(colorScheme: 'light' | 'dark', tokens: Record<string, string> = {}): ThemeSnapshot {
  ...
  return {
    preference: colorScheme, background: '', backgroundOpacity: 0.48, prompt: '',
...
describe('ThemePresenter', () => {
  ...
  it('applies and clears the user background with readable translucent surfaces', () => {
    const presenter = new ThemePresenter()
    presenter.apply({
      ...snapshot('dark'), background: 'data:image/png;base64,AA==', backgroundOpacity: 0.42,
    })
