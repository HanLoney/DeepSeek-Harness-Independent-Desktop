...
describe('ThemeRuntime', () => {
  ...
  it('adopts a published Host section without writing it back', () => {
    ...
    host.publish({ status: 'ready', value: {
      preference: 'dark', background: '', backgroundOpacity: 0.48, prompt: '',
  ...
  it('persists background and prompt settings and adopts Host updates', () => {
    const { theme, host } = make()
    theme.setBackground('data:image/png;base64,AA==')
    theme.setBackgroundOpacity(0.7)
    theme.setPrompt('Be concise.')
    expect(host.set).toHaveBeenNthCalledWith(1, 'background', 'data:image/png;base64,AA==')
    expect(host.set).toHaveBeenNthCalledWith(2, 'backgroundOpacity', 0.7)
    expect(host.set).toHaveBeenNthCalledWith(3, 'prompt', 'Be concise.')
    ...
    host.publish({ value: {
      preference: 'system', background: '', backgroundOpacity: 0.3, prompt: 'Host prompt.',
    }, revision: 2 })
    expect(theme.getTheme()).toMatchObject({ background: '', backgroundOpacity: 0.3, prompt: 'Host prompt.' })
  })

  it('rejects invalid personalization values before publishing or writing', () => {
    const { theme, host, events } = make()
    expect(() => { theme.setBackground('https://example.com/background.png') }).toThrow('base64 image data URL')
    expect(() => { theme.setBackgroundOpacity(1.1) }).toThrow('number from 0 to 1')
    expect(() => { theme.setPrompt('x'.repeat(12_001)) }).toThrow('at most 12000')
    expect(host.set).not.toHaveBeenCalled()
