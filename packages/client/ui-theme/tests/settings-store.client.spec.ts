...
describe('createAppearanceRowStore', () => {
  ...
  it('sync mirrors the preference and advances the revision', () => {
    const store = createAppearanceRowStore().create()
    store.actions.sync('dark', 'data:image/png;base64,AA==', 0.7, 'Be concise.', 0)
    expect(store.getSnapshot()).toEqual({
      preference: 'dark',
    ...
    store.actions.sync('light', '', 0.48, '', 2)
  ...
  it('revision guard drops stale and duplicate writes', () => {
    const store = createAppearanceRowStore().create()
    store.actions.sync('dark', '', 0.48, '', 3)
    store.actions.sync('system', '', 0.48, '', 2)
    store.actions.sync('system', '', 0.48, '', 3)
