...
import {
  DEFAULT_BACKGROUND_OPACITY, DEFAULT_PREFERENCE, THEME_SETTINGS_NAMESPACE, apply,
...
describe('ui-theme host', () => {
  it('registers, validates, and disposes the durable theme namespace with its fiber', async () => {
    ...
    expect(ctx.settings.get(ns)).toEqual({
      preference: 'dark', background: '', backgroundOpacity: DEFAULT_BACKGROUND_OPACITY, prompt: '',
    })
  ...
  it('adds the durable custom prompt to assembled model requests', async () => {
    const ctx = new Context()
    await ctx.plugin(MemorySettings).await()
    await ctx.plugin(SystemPrompt, { persona: '' }).await()
    await ctx.plugin({ apply }).await()
    const ns = settingsNamespace(THEME_SETTINGS_NAMESPACE)
    await ctx.settings.update(ns, { prompt: 'Keep answers concise.' })
    await vi.waitFor(async () => {
      expect(renderPrompt(await ctx.systemPrompt.assemble())).toContain('Keep answers concise.')
    })
  })
})
