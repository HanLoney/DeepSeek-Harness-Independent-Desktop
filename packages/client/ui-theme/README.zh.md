...
同一个 `ui-theme` 设置 namespace 还保存可选的背景图片数据 URL、背景显示透明度和个性化提示词。ThemeRuntime 会把三者放入每份快照；外观滑块会在每次输入时先更新缩略图和 ui-layout 的半透明基础表面，再持久化该值，Host 则把提示词作为顺序为 10 的 `ui-theme:custom-prompt` 系统提示词段落。0% 会隐藏背景，100% 会按完整强度显示背景。
...
非空的个性化提示词原文会作为顺序为 10 的 `ui-theme:custom-prompt` 系统提示词段落。agent loop 会在每个 step 前组装当前设置，并把生成的系统提示词记录到 `request/header`；空设置不产生内容。独占完整系统提示词的 persona 会抑制该段落。背景图片和配色不会进入模型请求。

#### Token 影响

仅在设置后产生，最多 12,000 个字符；配置存在期间，每次模型请求都会包含该提示词。

#### KV Cache 影响

个性化提示词不变时是稳定的系统提示词前缀。编辑它会改变已记录的请求头，并使提供方从该段落起的 prompt cache 不能复用。

## 已知限制与暂缓事项

- **背景图片保存在设置文档中**：外观设置接受 PNG、JPEG、WebP 或 GIF，不限制输入文件大小。无法直接装入设置值的图片会在本地缩小并编码为 WebP；尺寸过大的动态 GIF 会因此变成静态首帧。
- **个性化提示词为部署级设置**：它应用于当前 Host 的 agent，最多 12,000 个字符；如果某个 preset 的 persona 独占完整系统提示词，则会抑制它。每个 preset 的独立身份仍由 `dsh-persona` 管理。
