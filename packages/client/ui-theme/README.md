...
The same `ui-theme` settings namespace stores an optional background image data URL, its visible opacity, and a personalized prompt. ThemeRuntime includes all three values in each snapshot; the Appearance slider updates the thumbnail and ui-layout's translucent base surfaces on every input before persisting the value, while the Host contributes the prompt as the order-10 `ui-theme:custom-prompt` system-prompt section. Zero percent hides the background and 100 percent shows it at full strength.
...
The exact non-empty personalized prompt as the order-10 `ui-theme:custom-prompt` system-prompt section. The agent loop assembles the current setting before each step and records the resulting system prompt in `request/header`; an empty setting contributes nothing. A persona that owns the complete system prompt suppresses this section. The background image and palette never reach the model.

#### Token effect

Conditional and capped at 12,000 characters. The prompt is included in each model request while configured.

#### KV Cache effect

The personalized prompt is a stable system-prompt prefix while unchanged. Editing it changes the recorded request header and invalidates provider prompt-cache reuse from that section onward.

## Known Limitations and Deferred Work

- **Background images are stored inside the settings document** — the Appearance row accepts PNG, JPEG, WebP, or GIF without an input file-size limit. Images that do not fit the settings value are locally downscaled and encoded as WebP; this flattens an oversized animated GIF to its first frame.
- **Personalized prompts are deployment-wide** — the setting applies to agents using this Host and is limited to 12,000 characters; a preset whose persona owns the complete system prompt suppresses it. Per-preset identities remain owned by `dsh-persona`.
