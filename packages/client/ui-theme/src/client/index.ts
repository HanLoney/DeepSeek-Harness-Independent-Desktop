 ...
 * owns the live theme preference (light/dark/system), background image, and
 * personalized prompt, resolves `system` through `prefers-color-scheme`, and
 * publishes immutable snapshots; it never touches
 * the DOM — ui-layout's presenter consumes the resolved snapshot. The Host
 * settings scope loads and stores those preferences in the user-settings
...
import {
  DEFAULT_BACKGROUND_OPACITY, DEFAULT_PREFERENCE, isBackgroundDataUrl, isThemePreference, MAX_PROMPT_LENGTH,
...
export interface ThemeSnapshot {
  ...
  /** Persisted application background image data URL, or an empty string. */
  background: string
  /** Visible background strength from fully hidden (0) to fully visible (1). */
  backgroundOpacity: number
  /** Persisted model-facing custom prompt, or an empty string. */
  prompt: string
  /**
...
export class ThemeRuntime {
  ...
  /**
   * Persist and publish the optional application background image.
   * @param background - an empty string or a supported base64 image data URL.
   */
  setBackground(background: string): void {
    if (!isBackgroundDataUrl(background)) throw new TypeError('background must be a base64 image data URL')
    if (this.background === background) return
    this.background = background
    void this.host.set('background', background)
    this.publish()
  }

  /**
   * Persist and publish the visible background strength.
   * @param opacity - value from fully hidden (0) to fully visible (1).
   */
  setBackgroundOpacity(opacity: number): void {
    if (!Number.isFinite(opacity) || opacity < 0 || opacity > 1) {
      throw new TypeError('background opacity must be a number from 0 to 1')
    }
    if (this.backgroundOpacity === opacity) return
    this.backgroundOpacity = opacity
    void this.host.set('backgroundOpacity', opacity)
    this.publish()
  }

  /**
   * Persist and publish the optional model-facing custom prompt.
   * @param prompt - user-authored system-prompt text within the persisted length limit.
   */
  setPrompt(prompt: string): void {
    if (prompt.length > MAX_PROMPT_LENGTH) {
      throw new TypeError(`prompt must be at most ${String(MAX_PROMPT_LENGTH)} characters`)
    }
    if (this.prompt === prompt) return
    this.prompt = prompt
    void this.host.set('prompt', prompt)
    this.publish()
  }
  ...
  private adopt(): void {
    ...
    const changed = this.preference !== section.preference
      || this.background !== section.background
      || this.backgroundOpacity !== section.backgroundOpacity
      || this.prompt !== section.prompt
    if (!changed) return
...
export function apply(ctx: ClientContext): void {
  ...
  const sync = (snapshot: ThemeSnapshot): void => {
    bound?.sync(
      snapshot.preference, snapshot.background, snapshot.backgroundOpacity, snapshot.prompt, snapshot.revision,
    )
  }
  ...
  const injected = (actions: BoundActions<typeof store>): AppearanceRowInjected => {
    ...
    return {
      ...
      setBackgroundOpacity: (value) => { theme.setBackgroundOpacity(value) },
      setPrompt: (value) => { theme.setPrompt(value) },
    }
  }
