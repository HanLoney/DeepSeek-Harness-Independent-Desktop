...
describe('ModelSelect reasoning effort', () => {
  it('renders adapter metadata and submits the effort as part of the session selection', async () => {
    ...
    expect(screen.getByText('High').className).toContain('effortValue')
    const slider = screen.getByRole('slider', { name: '思考强度' })
    expect(slider.getAttribute('aria-valuetext')).toBe('High')
    expect(slider.getAttribute('title')).toBe('思考强度: High')

    fireEvent.change(slider, { target: { value: '2' } })
    expect(slider.getAttribute('aria-valuetext')).toBe('Max')
    expect(screen.getByText('Max').className).toContain('effortValue')
    expect(slider.getAttribute('title')).toBe('思考强度: Max - Largest budget')
  ...
  it('offers provider default only when the adapter does not configure a model default', () => {
    ...
    expect(slider.getAttribute('title')).toBe('思考强度: Default')
  })
