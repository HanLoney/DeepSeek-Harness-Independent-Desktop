...
当 `ctx.fs.sandboxMode` 表明提供方施加沙箱限制时，write/edit 会公开 `sandbox_permissions` 与 `justification`，并通过 `ctx.approval` 处理获批后的重试。若已知目标等于或窄于有效模式，该字段不起作用，修改会按常驻策略执行且不发起批准。真正更宽的目标要求配对的非空理由。策略归属方会贡献与具体能力无关的常驻策略；工具结果仍保留针对具体操作的拒绝与重试引导。
