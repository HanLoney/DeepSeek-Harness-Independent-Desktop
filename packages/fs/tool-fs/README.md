...
When `ctx.fs.sandboxMode` reports confinement, write/edit advertise `sandbox_permissions` and `justification` and resolve approved retries through `ctx.approval`. A recognized target equal to or narrower than the effective mode is inert: the mutation uses the standing policy without approval. A genuinely wider target requires a paired, non-empty reason. The policy owner contributes capability-neutral standing policy; the tool results retain operation-specific denial and retry guidance.
