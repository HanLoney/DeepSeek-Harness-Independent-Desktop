...
| `sandbox_permissions` | string enum | Advertised only when a sandboxing executor is mounted (`ctx.shell.sandboxMode` defined). The wider sandbox mode for a one-shot retry of a command the sandbox just denied — the narrowest wider mode that suffices, requiring `justification` and user approval through `ctx.approval` BEFORE execution. A recognized target equal to or narrower than the effective mode is ignored; an unapprovable wider request fails closed without running anything. |
| `justification` | string | Required with a genuinely wider `sandbox_permissions` target: one sentence for the user explaining why this exact command needs the wider access. Ignored with a redundant target. |
